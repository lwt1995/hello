//
// Created by lingweitao on 2023/3/2.
//
...
public:
    ...
    void post(const T &t) {
        std::unique_lock<std::mutex> lck(mutex_iostream);
        q.push(t);
    }
    ...
    // 队列取出元素
    bool dequeue(T &t) {
        std::unique_lock<std::mutex> lock(mutex_iostream); // 队列加锁
